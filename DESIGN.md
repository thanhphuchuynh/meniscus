...
### Named Rules
**The Hairline Rule.** Under increased contrast or forced colors, glass turns opaque and draws a 1px hairline edge; forced colors repaint it in the system's color. Nothing else on the page gains a border for contrast.
...
### Scales (range inputs)
A native range drawn as an engraved rule: a 1px rule line, graduation ticks every tenth beneath it, and a 2px vermilion fill up to the value. The thumb is a 16px stock circle with a 1.5px ink ring that turns vermilion and scales to 1.15 when active or focused. The label (Archivo 600) and the mono value with a faint unit sit on one baseline above; an optional faint hint below. The ticks are drawn in `ink-faint`, so they follow the theme. The media players' seek is the same engraved scale with a glass lens for its thumb: a 24 by 16px capsule carrying the thumb's 1.5px ink ring, vermilion while scrubbed or focused; in Chromium it bends the rule it rides.
...
### Iconography
Custom inline SVG icons on a 24px grid, 1.5px strokes, round caps and joins, no fills: the same single engraving weight as the figures. Rendered at 14 to 20px with `currentColor`. The skip glyphs (back and forward 5 seconds) are the reset arc with a stroked numeral 5, mirrored for forward; bare chevrons mean previous and next.

### Imagery
Imagery is period optics engravings only: Newton's *Opticks* (1704), Plates II and IV, public domain, duotoned to ink on stock for print and to light on dark for lantern, served as WebP. Each raster ships with a provenance sidecar (`.webp.json`) recording origin and processing, and is credited in the colophon. Images sit under glass as the thing being refracted, never as decoration. Plate VII's film is the same two plates: slow camera drifts rendered by `apps/site/scripts/generate-media.py` in both duotones, with a `.mp4.json` provenance sidecar, scored with the site's own synthesized étude.
